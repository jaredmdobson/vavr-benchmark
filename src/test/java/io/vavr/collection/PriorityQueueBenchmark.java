...
public class PriorityQueueBenchmark {
  ...
  public static class Base extends CollectionBenchmarkBase {
    ...
    scalaz.Heap<Integer> scalazImmutable;
    io.vavr.collection.PriorityQueue<Integer> vavrImmutable;
    fj.data.PriorityQueue<Integer, Integer> fjava_immutable;
    ...
    public void setup() {
      ...
      scalazImmutable = create(v -> scalaz.Heap.Empty$.MODULE$.<Integer>apply().insertAll(asScalaBuffer(v), SCALAZ_ORDER), asList(ELEMENTS), v -> v.size() == CONTAINER_SIZE);
      fjava_immutable = create(v -> fj.data.PriorityQueue.<Integer>emptyInt().enqueue(v), List.of(ELEMENTS).map(v -> P.p(v, v)).toJavaList(), ELEMENTS.length, v -> v.toList().length() == CONTAINER_SIZE);
      vavrImmutable = create(io.vavr.collection.PriorityQueue::of, ELEMENTS, ELEMENTS.length, v -> v.size() == CONTAINER_SIZE);
    }
  }
  ...
  public static class Enqueue extends Base {
    ...
    @Benchmark
    public Object scalaz_immutable() {
  ...
  public static class Dequeue extends Base {
    ...
    @Benchmark
    public Object scalaz_immutable() {
      scalaz.Heap<Integer> values = scalazImmutable;
    ...
    @Benchmark
    public Object fjava_immutable() {
      fj.data.PriorityQueue<Integer, Integer> values = fjava_immutable;
    ...
    @Benchmark
    public Object vavr_immutable() {
      io.vavr.collection.PriorityQueue<Integer> values = vavrImmutable;
